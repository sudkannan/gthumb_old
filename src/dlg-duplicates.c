		...
		gpointer             callback_data)
{
	...
	//fprintf	(stdout, "In read_callback data->current_path %s\n", data->current_path);
	...
	//fprintf (stdout, "In read_callback data->md5_bytes_read %u\n", data->md5_bytes_read);
	gnome_vfs_async_read (handle,
			data->md5_buffer + data->md5_bytes_read,
			BLOCKSIZE - data->md5_bytes_read,
			read_callback,
...
static void
process_nv_block (DialogData *data)
{
	size_t  pad, sum;
	char   *buffer;
		...
		gpointer callback_data)
{
	...
	if (result != GNOME_VFS_OK) {
        ...
        fprintf(stdout,"calling checksum \n");
		start_next_checksum (data);

		return;
	}
	//fprintf (stdout, "calling read_callback \n");
	...
	char *fname = extract_filename(data->current_path);
	//if(strstr(fname,"jpg")) 
	//fprintf(stdout,"filename %s \n", fname);
	//nvptr = p_c_nvread_len(fname,0,&data->md5_bytes_read);
	nvptr = p_c_nvread_len(fname,0,&fsize);
	assert(nvptr);
	fprintf(stdout,"nvptr %lu \n", (unsigned long)nvptr);
	data->md5_bytes_read = BLOCKSIZE;
	memcpy(data->md5_buffer, nvptr,BLOCKSIZE);
	fprintf(stdout,"after memcopy \n");	
	process_nv_block (data);
	fprintf(stdout,"after process_block \n");	
	md5_read_ctx (&data->md5_context, md5_sum);
	for (cnt = 0; cnt < 16; ++cnt) {
		char s[3];
		snprintf (s, 3, "%02x", md5_sum[cnt]);
	...
	idata = image_data_new (data->current_path, sum);
	fprintf(stdout,"after image_data_new \n");
	data->files = g_list_prepend (data->files, idata);
	fprintf(stdout,"after g_list_prepend \n");
	check_image (data, idata);
        fprintf(stdout,"after check_image \n");
	return;
	//if(fname) free(fname);
#else
	gnome_vfs_async_read (handle,
...
start_next_checksum (DialogData *data)
{
	...
	if ((data->queue == NULL) || data->stopped) {
		...
		fprintf(stdout,"Total time (checksum method): %ld\n", simulation_time(lstart,lend ));	
	...
	for(idx = 0; idx < itr; idx++) {
		data->md5_bytes_read = BLOCKSIZE;
		memcpy(data->md5_buffer, nvptr + (idx * BLOCKSIZE),BLOCKSIZE);
		process_nv_block (data);
	}
	//fprintf(stdout,"after process_block \n");	
	md5_read_ctx (&data->md5_context, md5_sum);
	...
	if (result == GNOME_VFS_ERROR_EOF) {
		if (data->queue != NULL) {
			if (! data->checking_file){
		        fprintf(stdout,"calling checksum after data->checking_file\n");
				start_next_checksum (data);
			 }
		} else
			scan_next_dir (data);
		...
		//fprintf(stdout,"after checksum: %ld\n", simulation_time(start,end ));	
...
search_dir_async (DialogData *data, const char *path)
{
	...
	//fprintf(stdout, "search_dir_async calling gnome_vfs_async_load_directory_uri \n");
